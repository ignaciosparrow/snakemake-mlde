...
rule call_variants_freebayes:
    ...
    output:
        vcf = output_path("variants/{sample}.vcf")
    log:
        output_path("logs/call_variants_freebayes_{sample}.log")
    params:
...
rule make_barcode_mutant_dictionary:
    input:
        bam = output_path("alignments/mapped_reads/{sample}.sorted.bam"),
        reference = config["inputs"]["ref"],
        script = "scripts/mutant_barcode_dictionary_maker_indel_clean.py",
        vcf = output_path("variants/{sample}.vcf")  # Add VCF as input
    output:
